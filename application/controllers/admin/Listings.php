...
class Listings extends CI_Controller {
	...
	public function index(){
        $data['title']="View Listings";
		$data['breadcrumb']=array('/'=>"Home","active"=>$data['title']);
		$data['datatable']=true;
		
		$result=$this->listing->gethospital();
		$data['hospitals']=$result['hospitals'];
		$this->template->load('listings','list',$data);
	}	
	...
	public function add(){
		...
		$this->template->load('listings','add',$data);
	}
	...
	public function addhospital(){
        if($this->input->post('addhospital')!==NULL){
            $data=$this->input->post();
            $data['slug']=generate_slug($data['name']);
            unset($data['addhospital']);
            $discountarray=array();
            if(isset($data['discount_id'])){
                ...
                foreach($discount_ids as $key=>$discount_id){
                    $discountarray[]=array("discount_id"=>$discount_id,"discount"=>$discounts[$key]);
                }
            }
            $working_hours=array();
            ...
            foreach($days as $key=>$day){
                if($schedule=='daily'){
                    $working_hours[]=array("day"=>$day,"open_time"=>$open_times[$key],"close_time"=>$close_times[$key]);
                }
                else{
                    $working_hours[]=array("day"=>$day,"24hours"=>1);
                }
            }
            ...
            else{$imgdata['thumb_image']='';}


            $result=$this->listing->addhospital($data,$discountarray,$working_hours,$imgdata);
            if($result['status']===true){
                $this->session->set_flashdata("msg",$result['message']);
            }
            else{
                $this->session->set_flashdata("msg_err",$result['message']);
            }
        }
